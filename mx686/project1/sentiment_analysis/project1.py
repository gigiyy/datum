...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    loss = label * (theta @ feature_vector + theta_0)
    return max(0, 1 - loss)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # Your code here
    hinge = [hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0) for i in range(0, feature_matrix.shape[0])]
    return np.average(hinge)
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    d = label * (current_theta @ feature_vector + current_theta_0)
    if (d < 1e-10):
...
def perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        ...
        # if np.isclose(start, theta, atol=0.01).all():
            # print("perceptron converged at round", t)
    return (theta, theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        # start = theta if t == 0 else sum / (t * m)
        # print(start)
        for i in get_order(feature_matrix.shape[0]):
            (theta, theta_0) = perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0 )
            sum += theta
            sum_0 += theta_0
        # end = sum / ((t + 1) * m)
        # if np.isclose(start, end, atol=0.01).all():
            # print("avg converged at round", t)
    total = feature_matrix.shape[0] * T
    return (sum / total, sum_0 / total)
...
def pegasos_single_step_update(
    ...
    """
    # Your code here
    d = label * (theta @ feature_vector + theta_0)
    if (d < 1 + 1e-10):
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        ...
        for i in get_order(feature_matrix.shape[0]):
            nt += 1
            eta = 1.0 / math.sqrt(nt)
            (theta, theta_0) = pegasos_single_step_update(feature_matrix[i], labels[i], L, eta, theta, theta_0 )
        # if np.isclose(start, theta, atol=0.01).all():
            # print("pegasos converged at round", t)
    return (theta, theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    # Your code here
    def fun1(vec):
        return 1 if theta @ vec + theta_0 > 1e-10 else -1
    return np.apply_along_axis(fun1, 1, feature_matrix)
...
def bag_of_words(texts, remove_stopword=True):
    ...
    print("stop word count", len(stopword))
    ...
    print("dictionary count", len(indices_by_word))
...
def extract_bow_feature_vectors(reviews, indices_by_word, binarize=True):
    ...
    if binarize:
        ...
        feature_matrix = np.vectorize(map)(feature_matrix)
