...
class RectangularRoom(object):
    ...
    def __index(self, pos):
        """
        return a tuple tile index from pos
        """
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        self.clean.add(self.__index(pos))
    ...
    def isPositionInRoom(self, pos):
        ...
        x = pos.getX()
        y = pos.getY()
        return False if x < 0 or x >= self.width or y < 0 or y >= self.height else True
...
if __name__ == "__main__":
    ...
    room.cleanTileAtPosition(Position(3.5, 2.8))
    print(room.isTileCleaned(int(pos1.getX()), int(pos1.getY())))
    print(room.isTileCleaned(3, 2))
    print(room.isTileCleaned(3, 3))
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        current = self.getRobotPosition()
        nextAngle = self.getRobotDirection()
        nextPos = current.getNewPosition(nextAngle, self.speed)
        while (not self.room.isPositionInRoom(nextPos)):
            nextAngle = random.randrange(0, 360)
            nextPos = current.getNewPosition(nextAngle, self.speed)
        self.room.cleanTileAtPosition(nextPos)
        self.setRobotDirection(nextAngle)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for _ in range(num_trials):
        allTicks.append(trial())
    return  float(sum(allTicks)) / num_trials
