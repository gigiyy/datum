...
fitData('6002/unit4/springData.txt')
pylab.show()
...
fitData3('6002/unit4/springData.txt')
pylab.show()
...
def checkFits(xVals, yVals):
    xVals = xVals*9.81  # convert mass to force (F = mg)
    a,b = pylab.polyfit(xVals, yVals, 1)  # fix y = ax + b
    # use line equation to graph predicted values
    estYVals = a*xVals + b
    return rSquare(yVals, estYVals)

def compare(fileName):
    xVals, yVals = getData(fileName)
    xVals = pylab.array(xVals)
    yVals = pylab.array(yVals)
    print("with full dataset: " + str(checkFits(xVals, yVals)))
    xVals = pylab.array(xVals[:-6])
    yVals = pylab.array(yVals[:-6])
    print("with pruned dataset: " + str(checkFits(xVals, yVals)))

compare('6002/unit4/springData.txt')
